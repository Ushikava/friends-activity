...
export async function toggleGameFavorite(id: number): Promise<{ is_favorite: boolean }> {
  const res = await apiFetch(`${API}/games/${id}/favorite`, { method: 'POST' })
  if (!res.ok) throw new Error('Ошибка')
  return res.json()
}

export async function fetchRandomGame(favoritesOnly = false): Promise<{ id: number } | null> {
  const params = favoritesOnly ? '?favorites_only=true' : ''
  const res = await apiFetch(`${API}/games/random${params}`)
